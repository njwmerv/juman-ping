...
from utility.game_constants import NAME, SCREEN_WIDTH, SCREEN_HEIGHT
...
class Level:
    ...
    # Magic Methods
    def __init__(self, level_data : str, assets : ImageLoader, factory : BlockFactory):
        self._terrain = []
        ...
        for block_data in data[terrain_key]:
            new_block : Block = factory.generate_block(x=block_data[X_KEY], y=block_data[Y_KEY], kind=block_data[KIND_KEY], width=block_data[WIDTH_KEY], height=block_data[HEIGHT_KEY])
        ...
        GOAL_DATA : dict[str, int] = data[goal_key]
        self._goal = factory.generate_block(x=GOAL_DATA[X_KEY], y=GOAL_DATA[Y_KEY], kind=GOAL_DATA[KIND_KEY])
        self._terrain_group.add(self._goal)
    ...
    def get_goal(self) -> pygame.Rect:
        return self._goal.rect